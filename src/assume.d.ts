...
interface BaseChain<T, K extends TypeTag> {
  ...
  equals(
    expected: Exclude<T, null | undefined | unknown>,
    msg?: string
  ): AssumptionFn<T, K>;
  instanceof<C extends new (...args: any[]) => any>(
    expected: C,
    msg?: string
  ): AssumptionFn<InstanceType<C>, InferTypeTagFromConstructor<C>>;
...
export declare class AssumingBus extends EventTarget {
  ...
  addEventListener<T = any>(event: string, fn: Listener<T>): () => void;
  removeEventListener<T = any>(event: string, fn: Listener<T>): void;
}
...
export declare function defRefHandler<R>(
  def: R,
  log?: ((err: unknown) => void) | boolean
): (err: unknown) => R;
export declare function defRefHandlerAsync<R>(
  def: R,
  log?: ((err: unknown) => void) | boolean
): (err: unknown) => Promise<R>;
export declare function assumedRoute<F extends AnyFn>(
