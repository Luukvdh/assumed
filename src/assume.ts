  ...
  const api = {
    ...
    emitOnPass(event: string, data?: unknown | (() => unknown)) {
      customEmits.push({
        cond: () => !failed,
        event,
        data: data === undefined ? () => (originalValues.length > 0 ? originalValues[0] : undefined) : typeof data === 'function' ? (data as () => unknown) : () => data,
      });
      return api;
    },
    /**
     ...
     */
    emitOn(condition: boolean | (() => boolean), event: string, data?: unknown | (() => unknown)) {
      const condFn: () => boolean = typeof condition === 'function' ? (condition as () => boolean) : () => !!condition;
      const dataFn: (() => unknown) | undefined = data === undefined ? () => (originalValues.length > 0 ? originalValues[0] : undefined) : typeof data === 'function' ? (data as () => unknown) : () => data;
...
export class AssumptionError extends Error {
  ...
  private buildRichMessage(originalMessage: string, inferenceText?: string): string {
...
function createAssumption<T>(value: T): AssumptionFn<T, 'unknown'> {
  ...
  const runAll = (): boolean => {
    try {
      ...
      const msg = isAssumptionError(e) ? (e as AssumptionError).message : String(e);

      // Only push once per error instance
      try {
        if (isAssumptionError(e)) {
          const mark = (e as any).__assume_event_pushed;
          if (!mark) {
            pushAssumeEvent({
              t: Date.now(),
              kind: 'refuted',
            ...
            (e as any).__assume_event_pushed = true;
          }
        } else {
          pushAssumeEvent({
            t: Date.now(),
  ...
  const base: BaseChain<T, any> = {
    ...
    andGuard<S extends T>(guard: (v: T) => v is S, msg?: string) {
      add(
        () => {
          if (!guard(value)) throw new Error(msg ?? 'Assumption failed (andGuard)');
        },
        'unknown',
        'andGuard',
      );
  ...
  const toStringChain = (): AssumptionFn<string, 'string'> =>
    Object.assign(runner as any, {
      ...
      trimmedNotEmpty(msg?: string) {
        add(
          () => {
            if (String(value).trim().length === 0) throw new Error(msg ?? 'Expected non-empty (trimmed)');
          },
          'string',
          'trimmedNotEmpty',
        );
      ...
      hasAnyOf(items: string[], msg?: string) {
        add(
          () => {
            ...
            if (!ok) throw new Error(msg ?? `Expected array to contain any of [${items.join(', ')}]`);
          },
          'array',
      ...
      hasEveryOf(items: string[], msg?: string) {
        add(
          () => {
            ...
            if (missing.length > 0) throw new Error(msg ?? `Missing required items: [${missing.join(', ')}]`);
          },
          'array',
  ...
  (runner as any).isDate = (msg?: string) => {
    add(
      () => {
        if (!(value instanceof Date)) throw new Error(msg ?? 'Expected Date instance');
      },
      'datetime',
      'isDate',
    );
      ...
      earlier(than: Date | number, msg?: string) {
        add(
          () => {
            ...
            if (!(v < t)) throw new Error(msg ?? `Expected earlier than ${new Date(t).toISOString()}`);
          },
          'datetime',
      ...
      later(than: Date | number, msg?: string) {
        add(
          () => {
            const v = (value as unknown as Date).getTime();
            const t = typeof than === 'number' ? than : than.getTime();
            if (!(v > t)) throw new Error(msg ?? `Expected later than ${new Date(t).toISOString()}`);
          },
          'datetime',
      ...
      wasBefore(than: Date | number, msg?: string) {
        add(
          () => {
            const v = (value as unknown as Date).getTime();
            const t = typeof than === 'number' ? than : than.getTime();
            if (!(v < t)) throw new Error(msg ?? `Expected before ${new Date(t).toISOString()}`);
          },
          'datetime',
      ...
      daysAgoExceeds(n: number, msg?: string) {
        add(
          () => {
            ...
            if (!(days > n)) throw new Error(msg ?? `Expected more than ${n} days ago`);
          },
          'datetime',
          `daysAgoExceeds(${n})`,
        );
      ...
      daysSinceAtLeast(n: number, msg?: string) {
        add(
          () => {
            ...
            const then = (value as unknown as Date).getTime();
            const days = (now - then) / (1000 * 60 * 60 * 24);
            if (!(days >= n)) throw new Error(msg ?? `Expected at least ${n} days since`);
          },
          'datetime',
          `daysSinceAtLeast(${n})`,
        );
 ...
 */
export function assumedRoute<F extends AnyFn>(def: Awaited<ReturnType<F>>, handler: F, log: ((err: unknown) => void) | boolean = false) {
  return (...args: Parameters<F>): ReturnType<F> => {
    try {
      ...
      if (out && typeof (out as any).then === 'function') {
        ...
        return (out as Promise<any>).catch((e) => onErr(enrichWithHandlerName(e, handler))) as ReturnType<F>;
      }
      ...
      if (isAsyncFn) {
        ...
        return onErrAsync(enrichWithHandlerName(e, handler)) as unknown as ReturnType<F>;
      }
...
export function assertIsArray(v: unknown, msg?: string): asserts v is unknown[] {
  assuming(that(v).isArray(), msg);
}

/** @internal Asserts the value is a plain object (non-null, non-array). */
export function assertIsObject(v: unknown, msg?: string): asserts v is Record<string, unknown> {
  assuming(that(v).isObject(), msg);
}

/** @internal Asserts the value is a DOM Element. */
export function assertIsElement(v: unknown, msg?: string): asserts v is Element {
  assuming(that(v).isElement(), msg);
}

/** @internal Asserts the value is a boolean. */
export function assertIsBoolean(v: unknown, msg?: string): asserts v is boolean {
...
export function assertNotNull<T>(v: T, msg?: string): asserts v is Exclude<T, null> {
  assuming(that(v).notNull(), msg);
}

/** @internal Asserts the value is undefined. */
export function assertIsUndefined(v: unknown, msg?: string): asserts v is undefined {
  assuming(that(v).isUndefined(), msg);
}

/** @internal Asserts the value is not undefined. */
export function assertNotUndefined<T>(v: T, msg?: string): asserts v is Exclude<T, undefined> {
...
export function assertNotNullOrUndefined<T>(v: T, msg?: string): asserts v is NonNullable<T> {
  assuming(that(v).notNullOrUndefined(), msg);
}
...
export function assertStringNotEmpty(v: unknown, msg?: string): asserts v is string {
  assuming(that(v).isString().notEmpty(), msg);
}
export function assertStringHasLength(v: unknown, len: number, msg?: string): asserts v is string {
  assuming(that(v).isString().hasLength(len), msg);
}
export function assertStringMinLength(v: unknown, n: number, msg?: string): asserts v is string {
  assuming(that(v).isString().minLength(n), msg);
}
export function assertStringMaxLength(v: unknown, n: number, msg?: string): asserts v is string {
  assuming(that(v).isString().maxLength(n), msg);
}
export function assertStringLengthBetween(v: unknown, min: number, max: number, msg?: string): asserts v is string {
  assuming(that(v).isString().lengthBetween(min, max), msg);
}
export function assertStringContains(v: unknown, needle: string | RegExp, msg?: string): asserts v is string {
  assuming(that(v).isString().contains(needle), msg);
}
export function assertStringStartsWith(v: unknown, prefix: string, msg?: string): asserts v is string {
  assuming(that(v).isString().startsWith(prefix), msg);
}
export function assertStringEndsWith(v: unknown, suffix: string, msg?: string): asserts v is string {
  assuming(that(v).isString().endsWith(suffix), msg);
}
export function assertStringMatches(v: unknown, re: RegExp, msg?: string): asserts v is string {
  assuming(that(v).isString().matches(re), msg);
}
export function assertStringEqualsIgnoreCase(v: unknown, expected: string, msg?: string): asserts v is string {
  assuming(that(v).isString().equalsIgnoreCase(expected), msg);
}
export function assertStringIncludesAny(v: unknown, needles: string[], msg?: string): asserts v is string {
  assuming(
    that(v)
      .isString()
      .includesAny(...needles),
...
export function assertStringIncludesAll(v: unknown, needles: string[], msg?: string): asserts v is string {
  assuming(
    that(v)
      .isString()
      .includesAll(...needles),
...
export function assertStringIsJSON(v: unknown, msg?: string): asserts v is string {
  assuming(that(v).isString().isJSON(), msg);
}

/** Assert a string is one of the provided options. */
export function assertStringOneOf(v: unknown, options: string[], msg?: string): asserts v is string {
  assuming(
    that(v)
      .isString()
      .that((s) => options.includes(s), msg ?? `Expected one of [${options.join(', ')}]`),
    msg,
  );
}
 ...
 */
export function assertStringEqualsIgnoreWhitespace(v: unknown, expected: string, ignoreAllWhitespace: boolean = false, msg?: string): asserts v is string {
  assuming(
    that(v)
      .isString()
      .that(
        (s) => {
          const norm = (x: string) => (ignoreAllWhitespace ? x.replace(/\s+/g, '') : x.trim());
          return norm(s) === norm(expected);
        },
        msg ?? `Expected string to equal (ignoring ${ignoreAllWhitespace ? 'all whitespace' : 'edges'})`,
      ),
    msg,
  );
}
...
export function assertArrayNotEmpty(v: unknown, msg?: string): asserts v is unknown[] {
  assuming(that(v).isArray().notEmpty(), msg);
}
export function assertArrayHasLength(v: unknown, len: number, msg?: string): asserts v is unknown[] {
  assuming(that(v).isArray().hasLength(len), msg);
}
export function assertArrayHasAnyOf(v: unknown, items: string[], msg?: string): asserts v is unknown[] {
  assuming(that(v).isArray().hasAnyOf(items), msg);
}
export function assertArrayHasEveryOf(v: unknown, items: string[], msg?: string): asserts v is unknown[] {
  assuming(that(v).isArray().hasEveryOf(items), msg);
}
export function assertArrayItemIsBoolean(v: unknown, index: number, msg?: string): asserts v is unknown[] {
  assuming(that(v).isArray().itemIsBoolean(index), msg);
}
export function assertArrayItemIsString(v: unknown, index: number, msg?: string): asserts v is unknown[] {
  assuming(that(v).isArray().itemIsString(index), msg);
}
export function assertArrayItemIsNumber(v: unknown, index: number, msg?: string): asserts v is unknown[] {
  assuming(that(v).isArray().itemIsNumber(index), msg);
}
export function assertArrayItemIsObject(v: unknown, index: number, msg?: string): asserts v is unknown[] {
  assuming(that(v).isArray().itemIsObject(index), msg);
}
export function assertArrayIncludesString(v: unknown, needle: string, msg?: string): asserts v is unknown[] {
  assuming(that(v).isArray().includesString(needle), msg);
}
export function assertArrayIncludesNumber(v: unknown, needle: number, msg?: string): asserts v is unknown[] {
  assuming(that(v).isArray().includesNumber(needle), msg);
}
export function assertArrayIncludesObject(v: unknown, needle: Record<string, unknown>, msg?: string): asserts v is unknown[] {
  assuming(that(v).isArray().includesObject(needle), msg);
}
export function assertArrayOnlyStrings(v: unknown, msg?: string): asserts v is string[] {
  assuming(that(v).isArray().onlyHasStrings(), msg);
}
export function assertArrayOnlyNumbers(v: unknown, msg?: string): asserts v is number[] {
  assuming(that(v).isArray().onlyHasNumbers(), msg);
}
export function assertArrayOnlyObjects(v: unknown, msg?: string): asserts v is Record<string, unknown>[] {
  assuming(that(v).isArray().onlyHasObjects(), msg);
}
export function assertArrayEveryTruthy(v: unknown, msg?: string): asserts v is unknown[] {
  assuming(that(v).isArray().everyIsTruthy(), msg);
}
export function assertArrayEveryFalsy(v: unknown, msg?: string): asserts v is unknown[] {
...
export function assertObjectHasKey<T extends object, K extends string>(obj: T, key: K, msg?: string): asserts obj is T & Record<K, unknown> {
  assuming(that(obj).isObject().hasKey(key), msg);
}

export function assertObjectHasKeys<T extends object, K extends readonly string[]>(obj: T, keys: K, msg?: string): asserts obj is T & Record<K[number], unknown> {
  assuming(
    that(obj)
      .isObject()
      .hasKeys(...keys),
...
export function assertObjectKeyEquals<T extends object, K extends string, V = unknown>(obj: T, key: K, expected: V, msg?: string): asserts obj is T & Record<K, V> {
  assuming(that(obj).isObject().keyEquals(key, expected), msg);
}

// Make all keys required and exclude `undefined` from their types
export function assertObjectAllKeysSet<T extends object>(obj: T, msg?: string): asserts obj is { [P in keyof T]-?: Exclude<T[P], undefined> } {
  assuming(that(obj).isObject().allKeysSet(), msg);
}

export function assertObjectAnyKeyNull(obj: unknown, msg?: string): asserts obj is Record<string, unknown> {
  assuming(that(obj).isObject().anyKeyNull(), msg);
}

/** Object keys must match exactly the provided list (no missing, no extra). */
export function assertObjectKeysExactly<K extends readonly string[]>(obj: unknown, keys: K, msg?: string): asserts obj is { [P in K[number]]: unknown } {
  const expected: Record<string, unknown> = Object.fromEntries((keys as readonly string[]).map((k) => [k, true]));
...
export function assertElementIsChildOf(el: unknown, parent: Element | string, msg?: string): asserts el is Element {
  assuming(
    that(el)
      .isElement()
      .that((e) => {
...
export function assertStringTrimmedNotEmpty(v: unknown, msg?: string): asserts v is string {
  assuming(
    that(v)
      .isString()
...
export function assertArrayUnique(v: unknown, msg?: string): asserts v is unknown[] {
  assuming(
    that(v)
      .isArray()
      .that((arr) => new Set(arr as any[]).size === (arr as any[]).length, msg ?? 'Expected array items to be unique'),
    msg,
  );
}
...
export function expectNotNullOrUndefined<T>(v: T, msg?: string): NonNullable<T> {
...
export const sureIsString = (v: unknown, msg?: string) => () => assertIsString(v, msg);
export const sureIsNumber = (v: unknown, msg?: string) => () => assertIsNumber(v, msg);
export const sureIsArray = (v: unknown, msg?: string) => () => assertIsArray(v, msg);
export const sureIsObject = (v: unknown, msg?: string) => () => assertIsObject(v, msg);
export const sureIsElement = (v: unknown, msg?: string) => () => assertIsElement(v, msg);
export const sureIsBoolean = (v: unknown, msg?: string) => () => assertIsBoolean(v, msg);
export const sureIsDate = (v: unknown, msg?: string) => () => assertIsDate(v, msg);
export const sureIsNull = (v: unknown, msg?: string) => () => assertIsNull(v, msg);
export const sureIsUndefined = (v: unknown, msg?: string) => () => assertIsUndefined(v, msg);
...
export const mustBe = {
  ...
  // targeted
  stringNotEmpty: (v: unknown, msg?: string) => () => assertStringNotEmpty(v, msg),
  stringHasLength: (v: unknown, len: number, msg?: string) => () => assertStringHasLength(v, len, msg),
  stringMinLength: (v: unknown, n: number, msg?: string) => () => assertStringMinLength(v, n, msg),
  stringMaxLength: (v: unknown, n: number, msg?: string) => () => assertStringMaxLength(v, n, msg),
  stringLengthBetween: (v: unknown, min: number, max: number, msg?: string) => () => assertStringLengthBetween(v, min, max, msg),
  stringContains: (v: unknown, needle: string | RegExp, msg?: string) => () => assertStringContains(v, needle, msg),
  stringStartsWith: (v: unknown, prefix: string, msg?: string) => () => assertStringStartsWith(v, prefix, msg),
  stringEndsWith: (v: unknown, suffix: string, msg?: string) => () => assertStringEndsWith(v, suffix, msg),
  stringMatches: (v: unknown, re: RegExp, msg?: string) => () => assertStringMatches(v, re, msg),
  stringEqualsIgnoreCase: (v: unknown, expected: string, msg?: string) => () => assertStringEqualsIgnoreCase(v, expected, msg),
  stringIncludesAny: (v: unknown, needles: string[], msg?: string) => () => assertStringIncludesAny(v, needles, msg),
  stringIncludesAll: (v: unknown, needles: string[], msg?: string) => () => assertStringIncludesAll(v, needles, msg),
  stringIsJSON: (v: unknown, msg?: string) => () => assertStringIsJSON(v, msg),
  stringTrimmedNotEmpty: (v: unknown, msg?: string) => () => assertStringTrimmedNotEmpty(v, msg),
  stringOneOf: (v: unknown, options: string[], msg?: string) => () => assertStringOneOf(v, options, msg),
  stringEqualsIgnoreWhitespace: (v: unknown, expected: string, ignoreAllWhitespace?: boolean, msg?: string) => () => assertStringEqualsIgnoreWhitespace(v, expected, ignoreAllWhitespace, msg),

  arrayNotEmpty: (v: unknown, msg?: string) => () => assertArrayNotEmpty(v, msg),
  arrayHasLength: (v: unknown, len: number, msg?: string) => () => assertArrayHasLength(v, len, msg),
  arrayHasAnyOf: (v: unknown, items: string[], msg?: string) => () => assertArrayHasAnyOf(v, items, msg),
  arrayHasEveryOf: (v: unknown, items: string[], msg?: string) => () => assertArrayHasEveryOf(v, items, msg),
  arrayItemIsBoolean: (v: unknown, index: number, msg?: string) => () => assertArrayItemIsBoolean(v, index, msg),
  arrayItemIsString: (v: unknown, index: number, msg?: string) => () => assertArrayItemIsString(v, index, msg),
  arrayItemIsNumber: (v: unknown, index: number, msg?: string) => () => assertArrayItemIsNumber(v, index, msg),
  arrayItemIsObject: (v: unknown, index: number, msg?: string) => () => assertArrayItemIsObject(v, index, msg),
  arrayIncludesString: (v: unknown, needle: string, msg?: string) => () => assertArrayIncludesString(v, needle, msg),
  arrayIncludesNumber: (v: unknown, needle: number, msg?: string) => () => assertArrayIncludesNumber(v, needle, msg),
  arrayIncludesObject: (v: unknown, needle: Record<string, unknown>, msg?: string) => () => assertArrayIncludesObject(v, needle, msg),
  arrayOnlyStrings: (v: unknown, msg?: string) => () => assertArrayOnlyStrings(v, msg),
  arrayOnlyNumbers: (v: unknown, msg?: string) => () => assertArrayOnlyNumbers(v, msg),
  arrayOnlyObjects: (v: unknown, msg?: string) => () => assertArrayOnlyObjects(v, msg),
  arrayEveryTruthy: (v: unknown, msg?: string) => () => assertArrayEveryTruthy(v, msg),
  arrayEveryFalsy: (v: unknown, msg?: string) => () => assertArrayEveryFalsy(v, msg),
  ...
  objectHasKeys:
    <T extends object, K extends readonly string[]>(obj: T, keys: K, msg?: string) =>
    () =>
      assertObjectHasKeys(obj, keys, msg),
  objectKeyEquals:
    <T extends object, K extends string, V = unknown>(obj: T, key: K, expected: V, msg?: string) =>
    () =>
  ...
  objectAnyKeyNull: (obj: unknown, msg?: string) => () => assertObjectAnyKeyNull(obj, msg),
  objectKeysExactly:
    <K extends readonly string[]>(obj: unknown, keys: K, msg?: string) =>
    () =>
      assertObjectKeysExactly(obj, keys, msg),
  elementIsChildOf: (el: unknown, parent: Element | string, msg?: string) => () => assertElementIsChildOf(el, parent, msg),
} as const;
...
export function reportAssertions(v: unknown, filter?: Array<string | RegExp>): AssertionResult[] {
  const entries = !filter || filter.length === 0 ? assertionsCatalog : assertionsCatalog.filter((e) => filter.some((f) => (typeof f === 'string' ? e.name === f : (f as RegExp).test(e.name))));
...
function likeArray(a: unknown[], b: unknown[], o: Required<LikeOptions>): boolean {
...
function likeObject(a: Record<string, unknown>, b: Record<string, unknown>, o: Required<LikeOptions>): boolean {
...
export function isAlotLike(a: unknown, b: unknown, opts?: LikeOptions): boolean {
  const strict = {
    keys: 'exact' as const,
    checkValues: true,
